...
public class Pong extends Canvas implements KeyListener, Runnable
{
	...
	public Pong()
	{
		...
		rightPaddle = new Paddle(20,200,10,40,Color.orange,2);
		leftScore =0;
		rightScore = 0;
   ...
   public void paint(Graphics window)
   {
		...
		{
			hitLeftPaddle = true;
			if(ball.getX()<=leftPaddle.getWidth()+leftPaddle.getX()-Math.abs(ball.getXSpeed())) {
				ball.setYSpeed(-ball.getYSpeed());
			}
			else
		...
		{
			hitLeftPaddle = true;
			if(ball.getX()<=rightPaddle.getWidth()+rightPaddle.getX()-Math.abs(ball.getXSpeed())) {
				ball.setYSpeed(-ball.getYSpeed());
			}
			else
				ball.setXSpeed((-ball.getX()));
